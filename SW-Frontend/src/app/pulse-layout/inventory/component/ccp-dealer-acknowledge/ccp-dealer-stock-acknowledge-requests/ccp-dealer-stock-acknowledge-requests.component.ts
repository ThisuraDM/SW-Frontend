...
@Component({
  selector: 'SW-ccp-dealer-stock-acknowledge-requests',
  templateUrl: './ccp-dealer-stock-acknowledge-requests.component.html',
  styleUrls: ['./ccp-dealer-stock-acknowledge-requests.component.scss']
})
export class CcpDealerStockAcknowledgeRequestsComponent implements OnInit {

  @Output() transferSelect = new EventEmitter<Content>();
  @Output() viewSummaryCCPDealerAcknowledge = new EventEmitter<Content>();
  ...
  public errorMessage = 'No results to display for the search made.';
  ...
  onSelectAction(item: Content) {
    if(item.transferStatus.toUpperCase() == 'RECEIVED'){
      this.viewSummaryCCPDealerAcknowledge.emit(item);
    }else{
      this.store.dispatch(InventoryStoreDetails.setstockTransferDetails({ input: this.stockTransferDetails }));
      item.navigateOption = this.getActionButton(item.transferStatus, item.transferToStoreId === this.selectedStore)
  ...
  private initForm(outletId:string): void {
    this.ownStoreSearchForm = new FormGroup({
      ...
      outlet: new FormControl(outletId, { validators: [] }),
    });
  }
  ...
  loadPage(page: number) {
    ...
    this.bcSearchStockService.getTransferDetails(
      this.localStorageService.get(StorageSettings.LOGIN_NAME),formattedfromDate,formattedtoDate, this.ownStoreSearchForm.controls.outlet.value,page - 1,this.pageSize,this.userRole,
            this.ownStoreSearchForm.controls.requestID.value, this.ownStoreSearchForm.controls.status.value,false
            ).subscribe(res => {
      if (res) {
  ...
  private retrieveTransferStatus(): void {
    this.bcSearchStockService.getTransferStatusDealer().subscribe(
      response => {
        if (response?.length) {
